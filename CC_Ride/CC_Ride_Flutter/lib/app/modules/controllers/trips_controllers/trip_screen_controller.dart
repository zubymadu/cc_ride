...
class TripScreenController extends GetxController with GetTickerProviderStateMixin {
  ...
  void onInit() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      isLoading = true;
      update();
      tripListApi(status: "Active");
    });
    super.onInit();
  }

  late TabController tabController = TabController(length: 2, vsync: this);
  final RxList _tabText = ["Post", "Request"].obs;
  List get tabText => _tabText;
  set tabText(List value) => _tabText.value = value;

  late TabController postTripTabController = TabController(length: 3, vsync: this);
  // final RxList _postTripTabText = ["Active", "Recent", "Cancelled"].obs;
  final RxList _postTripTabText = ["Active", "Completed", "Cancelled"].obs;
  List get postTripTabText => _postTripTabText;
  set postTripTabText(List value) => _postTripTabText.value = value;

  final RxBool _isLoading = true.obs;
  bool get isLoading => _isLoading.value;
  set isLoading(bool value) => _isLoading.value = value;

  TripListApiModel? tripListActiveApiModel;
  TripListApiModel? tripListRecentApiModel;
  TripListApiModel? tripListCancelledApiModel;
  ...
  Future tripListApi({required String status}) async {
    ...
    try {
      String url = Confing.baseurl + Confing.tripList;

      debugPrint("============= trip List Api url ============== $url");
      debugPrint("============= trip List Api Body ============= $body");
      ...
      debugPrint("=========== trip List Api response =========== ${response.body}");
      ...
      if (response.statusCode == 200) {
        if (status == "Active") {
          debugPrint("=========== Active ===========");
          tripListActiveApiModel = tripListApiModelFromJson(response.body);
          if (tripListActiveApiModel != null) {
            isLoading = false;
            update();
          }
        } else if (status == "Recent") {
          debugPrint("=========== Recent ===========");
          tripListRecentApiModel = tripListApiModelFromJson(response.body);
          if (tripListRecentApiModel != null) {
            isLoading = false;
            update();
          }
        } else if (status == "Cancelled") {
          debugPrint("=========== Cancelled ===========");
          tripListCancelledApiModel = tripListApiModelFromJson(response.body);
          debugPrint("=========== result =========== ${tripListCancelledApiModel!.result}");
          if (tripListCancelledApiModel != null) {
            isLoading = false;
            update();
          }
        }
  ...
  final RxBool _requestIsLoading = true.obs;
  bool get requestIsLoading => _requestIsLoading.value;
  set requestIsLoading(bool value) => _requestIsLoading.value = value;

  TripRequestListApiModel? tripRequestListApiModel;

  Future tripRequestListApi() async {
    requestIsLoading = true;
    update();
    final userLogin = getData.read("userLogin");
    if (userLogin == null) { requestIsLoading = false; update(); return; }
    Map body = {"uid": userLogin["id"]};

    try {
      String url = Confing.baseurl + Confing.requestList;

      debugPrint("============= Trip Request List Api url ============== $url");
      debugPrint("============= Trip Request List Api Body ============= $body");
      ...
      debugPrint("=========== Trip Request List Api response =========== ${response.body}");

      var data = jsonDecode(response.body);

      if (response.statusCode == 200) {
        if (data["Result"] == "true") {
          tripRequestListApiModel = tripRequestListApiModelFromJson(response.body);
          if (tripRequestListApiModel!.result == "true") {
            requestIsLoading = false;
            update();
            return data;
          }
        } else {
          showToastMessage("${tripListActiveApiModel!.responseMsg}");
          return data;
        }
      } else {
        showToastMessage("Somthing went wrong!.....");
      ...
      debugPrint("============= Trip Request List Api Error ============= $e");
    }
  }

  Widget tripDetailsData({
